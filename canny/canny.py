...
G = cv2.GaussianBlur(img,(3,3),sigma)                           #gaussian low pass filter

sobelout = numpy.zeros(imgdata.shape)                                      #empty image
gradx = numpy.array(sobelout, dtype = float)                        
grady = numpy.array(sobelout, dtype = float)

sobel_x = numpy.array([[-1,0,1],
           [-2,0,2],
           [-1,0,1]],dtype=numpy.float)
sobel_y = numpy.array([[-1,-2,-1],
           [0,0,0],
           [1,2,1]],dtype=numpy.float)


width = imgdata.shape[1]
height = imgdata.shape[0]

grad_x = cv2.filter2D(G, 1, sobel_x)
grad_y = cv2.filter2D(G, 1, sobel_y)

grad_x = numpy.array(grad_x,dtype=float)
grad_y = numpy.array(grad_y,dtype=float)
